...
class HopfieldNet():
    ...
    def train(self, data):
        ...
        if isinstance(self.thr_val, (float, int)):
            ...
            raise ValueError('Threshold should be int/float or numpy.ndarray')
    ...
    def recall(self, input, n_steps=None, synchronous=False,
               store_energy=False, warm_start=False):
        ...
        if not hasattr(self, 'E') or not store_energy:
            self.E = []

        if synchronous:
            for i in range(n_steps):
                self.state = np.where(np.dot(self.W, self.state) > self.thr, 1, -1)
                self.E.append(self.energy(self.state))
        else:
            for i in range(n_steps):
                sel = np.random.randint(low=0, high=self.n_units)
                act = np.dot(self.W[sel, :], self.state)
                self.state[sel] = 1 if act > self.thr[sel] else -1
                self.E.append(self.energy(self.state))
...
class simulation_0():
    def __init__(self, train_data, input_data=None):
        ...
        self.side = int(np.sqrt(self.train_data.shape[1]))
    ...
    def plot(self, state):
        with self.out:
            ...
            # network state
            try:
                sns.heatmap(state.reshape(self.side, self.side), ax=ax[0, 1], vmin=-1, vmax=1, cmap='RdBu_r')
            except:
                pass

            # weights
            sns.heatmap(self.net.W, ax=ax[1, 0], xticklabels=False, yticklabels=False, cmap='RdBu_r')
    ...
    def update_button(self, b):
        state = self.net.recall(self.input_data, n_steps=ne_widget.value, synchronous=False,
                                store_energy=True, warm_start=True)
...
class simulation():
    ...
    def plot(self, state):
        with self.out:
            ...
            # pattern to retrieve
            pattern = letter_A if sel_input.value == 'A' else letter_B
            sns.heatmap(pattern, ax=ax[0, 0], vmin=-1, vmax=1, cmap='RdBu_r')
    ...
    def generate_random_input(self, b):
        self.net = HopfieldNet()
        self.net.train(self.train_data)
        input_data = letter_A.reshape(81) if sel_input.value == 'A' else letter_B.reshape(81)
